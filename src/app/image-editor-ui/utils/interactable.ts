...
interface InteractableConfig {
    ...
    forceInitialAspectRatio?: boolean;
    minWidth: number;
    minHeight: number;
    transformEl?: boolean;
...
export class Interactable {
    ...
    private syncCurrentRectWithEl() {
        const [, left, top, rotation = '0'] = this.config.el.style.transform.match(/translate\(([\-0-9\.]+)px,.([\-0-9\.]+)px\)(.rotate\(([\-0-9\.]+)rad\))?/);
        this.currentRect = {
        ...
        if ( ! this.config.aspectRatio && this.config.forceInitialAspectRatio) {
            this.config.aspectRatio = this.currentRect.width / this.currentRect.height;
        }
    }
    ...
    public constrainRect(currentRect: RectSizeAndPos, prevRect: RectSizeAndPos): RectSizeAndPos {
       ...
       if (this.config.minWidth || this.config.minHeight) {
           ...
           if (this.config.aspectRatio) {
               min = Interactable.calculateNewSizeFromAspectRatio(this.config.aspectRatio, this.config.minWidth, this.config.minHeight);
           } else {
